...
> ⚠️ 当前版本没有内置凭据，AppKey/SecretKey 需要在 app 里手动填写。填写位置：「添加书源」对话框选择百度网盘后，点「高级选项（自填 AppKey/SecretKey）」。以后也可以直接把凭据内置进应用，用户就无需自己注册。

**在 app 中添加**

1. 类型选「百度网盘」；
2. 「根目录」填挂载目录（见下方目录说明）；
3. 展开「高级选项」，填入自己的 AppKey / SecretKey；
...
3. 如有自建 APP ID，展开「高级选项」填写（当前版本无内置，留空会提示未配置，需要自行到 115 开放平台申请）；
