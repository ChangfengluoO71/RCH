...
| 平台                  | 文件                          | 说明                           |
| ------------------- | --------------------------- | ---------------------------- |
| Windows 10 / 11 x64 | `RCH-0.6.1-windows-x64.exe` | Windows 桌面版                  |
...
- WebDAV 书源启动后**无法建立会话**（会话探测打在书源根目录上，部分服务返回 405 被误判为登录失败）
  ⇒ 该源封面任务一直停在"等待"、点刷新也没反应。现已不再因此中断登录。
- 浏览/离线索引时**文件大小未写入索引**，导致大文件封面判定"拿不到大小"而失败。

新增：

- 扫描的后台自愈：索引里存在"大小未知"的文件时，会按既有的目录重查周期自动重新列目录
  （WebDAV 若仍显示失败，点一次「离线索引」可立即补齐）。

完整说明见[版本更新说明](docs/releases/release_notes_v0.6.1.md)。
