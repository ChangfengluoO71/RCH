...
- **打开快了很多**：EPUB 从半分钟到不到 1 秒；ZIP/CBZ 打开不再随页数变慢；MOBI 第二次打开几乎瞬间；默认改为流式阅读（点开就看）。
- **封面终于稳了**：修掉"详情页有封面、海报墙显示获取失败"；看过详情页后海报墙会立即刷新；封面失败可在书源页点「刷新」真正重试；80 MB 以上大漫画的封面不再永久失败。
- **新增「阅读渲染宽度」**：省流 1080 / 标准 1600 / 跟随屏幕（标准档与旧版完全一致，不会让已有缓存失效）。
- **缓存更省心**：整本下载缓存有 2 GiB 上限并自动清理最旧的；115/夸克显示**真实文件名**；扫描状态栏去掉多余按钮（扫描自动运行）。
