...
> v0.6.0：默认改为**流式阅读**（点开就能看，网络不稳时自动退回整本下载）；新增
> **「设置 → 阅读渲染宽度」**（省流 1080 / 标准 1600 / 跟随屏幕）——想省流量或让翻页更轻快选"省流"，
> 在意清晰度保持"标准"。详见[用户手册](docs/user-guide.md)。
...
<p align="center"><sub>海报墙：封面 + 文件名 + 体积；顶部一行显示扫描与封面进度（可用 / 等待 / 进行中 / 失败）</sub></p>
...
v0.5.4 提供一个不依赖在线元数据 Provider 的通用目录刮削流程：
...
识别结果会优先显示在漫画详情页；作者、系列和稳定资源属性也会进入标签管理。设置中的刮削入口只保留**重新刮削**，用于用户调整文件夹或目录结构后重新运行。
...
设置项按类别折叠，找起来更快：**阅读 / 书源与网络 / 缓存与存储 / 同步与备份 / 外观与布局**。
...
| 想做什么 | 去哪里 |
| --- | --- |
| 调整清晰度 / 省流量 | 阅读 → **阅读渲染宽度**（省流 1080 / 标准 1600 / 跟随屏幕） |
| 读完自动清理整本包 | 缓存与存储 → **阅读完成后自动删除整包** |
| 清理缓存、看占用 | 缓存与存储 |
| 添加 / 编辑远程书源 | 书源与网络 |
...
> v0.6.0：封面若显示"获取失败"，可在该**书源页面点「刷新」**重新排队重试（会提示"已重新排队 N 张失败封面"）；
> 115 / 夸克书源里的「原文件名」显示**真实文件名**而不再是一长串数字 ID；支持**扫码登录**。

添加书源时可以直接**扫码获取 Cookie**（无需申请 APP ID），根文件夹 ID 也能用「选择文件夹」自动填入：
...
> v0.6.0：整本下载缓存新增 **2 GiB 上限**，超出后自动删除**最旧**的整本包并始终保留最新在读的；
> 也可在设置里开启「**阅读完成后自动删除整包**」。清整本缓存**不会**连带清掉封面缓存，反之亦然。
* AI 超分结果
* 临时文件
