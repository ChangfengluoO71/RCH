...
| 平台                  | 文件                          | 说明                           |
| ------------------- | --------------------------- | ---------------------------- |
| Windows 10 / 11 x64 | `RCH-0.6.0-windows-x64.exe` | Windows 桌面版                  |
...
- 远程书源海报墙与扫描（含扫描与封面进度）；夸克扫码登录；115 扫码获取 Cookie（无需 APP ID）。
- 「阅读渲染宽度」设置（省流 1080 / 标准 1600 / 跟随屏幕）；「阅读完成后自动删除整包」。
- 封面重抓：书源页面点「刷新」即可重新排队失败的封面；设置项按类别折叠。

改进：

- EPUB 打开 30 秒级降到 1 秒内；ZIP / CBZ 打开耗时不再随条目数增长；PDF、MOBI 改为按需读取。
- 默认流式阅读（点开即可读，失败回退整本）；115 与夸克显示真实文件名。
- 整本下载缓存上限 2 GiB，超出自动清理最旧的。

修复：

- 海报墙「获取失败」而详情页有封面、详情页看图后海报墙不刷新、失败封面点刷新无效、
  大体积文件封面永久失败、Windows 版缺少 PDF 组件等问题。

完整说明见[版本更新说明](docs/releases/release_notes_v0.6.0.md)。
...
设置项按类别折叠为五组：**阅读 / 书源与网络 / 缓存与存储 / 同步与备份 / 外观与布局**。
