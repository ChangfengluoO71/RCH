...
| 平台                  | 文件                          | 说明                           |
| ------------------- | --------------------------- | ---------------------------- |
| Windows 10 / 11 x64 | `RCH-0.5.4-windows-x64.exe` | Windows 桌面版                  |
...
- 从本地目录或已经建立的远程目录索引生成 Catalog Snapshot，再自动运行离线智能刮削。
- `Ready` 且无冲突的结果会写入作品名、作者、系列等元数据，并同步生成用户可理解的资源标签。
- 标签词汇收敛为 `Chinese`、`无修正`、`高清`；不再显示“数字版”等无实际价值的内部标记。
- 漫画详情页同时显示识别结果和原文件名，原文件名可直接选择复制。
- 新增/编辑书源无需重启；115/夸克根目录修改即时生效；确认删除后会清理关联数据与缓存。

完整说明见[版本更新说明](docs/releases/release_notes_v0.5.4.md)。
