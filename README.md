...
* 🧠 **离线智能刮削** — 只使用目录快照解析作品名、作者、章节和资源属性，Ready 结果自动进入元数据与标签
...
* 🔌 **可选插件：EH 订阅** — 设置页「EH 订阅（可选插件）」栏：按可编辑规则（搜索语法/评分/标题标记/分时间段下载数）筛选种子并保存到指定文件夹；主站域名可配 + 连通性预检；默认关闭、只写该目录，不改动书源与阅读数据（边界见 SPEC 第 10 节）
* 🎲 **随机阅读** — 「最近阅读」栏提供「随机一本」；读到最后一页延迟 3 秒提示，可直接换一本或退回详情页
...
| 平台                  | 文件                          | 说明                           |
| ------------------- | --------------------------- | ---------------------------- |
| Windows 10 / 11 x64 | `RCH-0.6.2-windows-x64.exe` | Windows 桌面版                  |
...
- **进入云端书源更快**：复用启动已建立的会话，浏览过的书源再次进入基本无需等待（此前每次进入都会重新登录）。
- 建立会话少两次往返；并发档位回到真实水平，扫描与封面随之更快。
- 后台扫描在阅读/浏览时主动让路，空闲时不减速。

调整：

- 点「更新」直接进入**可见的下载与安装界面**（含安装包保存位置），下载完成自动安装；Windows 显示安装向导。
- 设置页新增独立的「关于与更新」折叠栏。

完整说明见[版本更新说明](docs/releases/release_notes_v0.6.2.md)。
...
**[智能刮削](docs/user-guide.md#45-离线智能刮削与自动化)** → **漫画详情页核对** → **标签管理检查**
...
对于 ZIP / CBZ / EPUB / PDF / MOBI 等格式，RCH 只读取打开所需的目录（或页表），页面内容按需读取，
不在打开时完整解压或下载整个文件；MOBI 第二次打开会复用已缓存的页表。

因此，大型漫画压缩包与远程大文件都可以直接进入阅读。

---

### 阅读器

> v0.6.0：默认使用流式阅读，点开即可阅读，流式不可用时自动回退整本下载。
> 新增「阅读渲染宽度」设置：省流 1080 / 标准 1600 / 跟随屏幕。详见[用户手册](docs/user-guide.md)。
...
<p align="center">
  <img src="docs/images/v0.6.0/library-wall-pdf.jpg" width="300" alt="海报墙：PDF 漫画，显示封面、文件名与体积">
  <img src="docs/images/v0.6.0/library-wall-epub.jpg" width="300" alt="海报墙：EPUB 漫画，显示封面、文件名与体积">
</p>

<p align="center"><sub>海报墙显示封面、文件名与体积；顶部显示扫描与封面进度（可用 / 等待 / 进行中 / 失败）</sub></p>

点开任意一本进入漫画详情页：开始阅读、自定义封面、标记已读。

<p align="center">
  <img src="docs/images/v0.6.0/book-detail.jpg" width="300" alt="漫画详情页：封面、开始阅读、自定义封面、已读">
</p>
...
* 批量标记
* 文件夹递归操作
* **阅读统计**（最多阅读的漫画 / 系列 / 标签 / 作者 / 类别 Top10，点击直达详情或对应标签）
* **清理失效漫画数据**（在线核对远程书源 + 墓碑判定：远程删除/本地删除的漫画连同记录、元数据、标签与缓存一并清理；「清空全部缓存」可分别确认是否清空最近阅读记录与阅读统计）

---

### 🧠 离线智能刮削

RCH 提供不依赖在线元数据 Provider 的通用目录刮削流程：
...
刮削只读取本地 catalog/index 中已有的文字和目录关系，不打开漫画内容，不读取压缩包/页面/封面，也不为了补上下文访问远程书源。远程漫画必须先存在于本地索引中，才能使用已有文件名和目录快照参与刮削。

识别结果会优先显示在漫画详情页；作者、系列和稳定资源属性也会进入标签管理。设置中的刮削入口只保留**重新刮削**（`设置 → 书源与网络`），用于调整文件夹或目录结构后重新运行。
...
设置项按类别折叠为六组：**阅读 / 书源与网络 / 缓存与存储 / 同步与备份 / 外观与布局 / 关于与更新**。

<p align="center">
  <img src="docs/images/v0.6.0/settings.jpg" width="300" alt="设置页：按类别折叠的五组设置">
</p>
...
| 操作 | 位置 |
| --- | --- |
| 调整渲染清晰度或省流量 | 阅读 → **阅读渲染宽度**（省流 1080 / 标准 1600 / 跟随屏幕） |
| 阅读完成后自动清理整本包 | 缓存与存储 → **阅读完成后自动删除整包** |
| 查看缓存占用与清理 | 缓存与存储 |
| 添加或编辑远程书源 | 书源与网络 |
| 检查更新 / 下载并安装新版本 | 关于与更新 |
...
> v0.6.0：远程书源支持海报墙与扫描，书架顶部显示扫描进度与封面进度（可用 / 等待 / 进行中 / 失败）。
> 封面显示「获取失败」时，在该书源页面点「刷新」会重新排队失败的封面。
> 115 与夸克书源显示真实文件名；两者均支持扫码登录，保存的凭据在启动时自动重连。

添加书源时可扫码获取 Cookie（115 无需申请 APP ID），根文件夹 ID 可用「选择文件夹」自动填入：

<p align="center">
  <img src="docs/images/v0.6.0/add-source-115.jpg" width="300" alt="添加书源：115 网盘，扫码获取 Cookie 与自动填根文件夹 ID">
</p>
...
当远程服务支持 HTTP Range 时，可以边下载边阅读；不支持时回退整本下载并进入本地缓存。
...
离线智能刮削使用的就是这份本地快照。它不会因为缺少封面、章节或元数据而自动刷新远程目录；需要新的文件名或父级关系时，应先按你的意图在线浏览/重建索引，再点击“重新刮削”。
...
> v0.6.0：整本下载缓存上限为 2 GiB，超出后自动删除最旧的整本包，并保留最新在读的；
> 设置中可开启「阅读完成后自动删除整包」。整本缓存与封面缓存相互独立，清理其一不影响另一个。
* AI 超分结果
* 临时文件
