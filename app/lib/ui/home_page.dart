...
class _HomePageState extends State<HomePage> {
  ...
  String? _detailTag;
  String? _tagManagerProjectionKey;
  List<(String, int, int)> _cachedTagStats = const [];
  Set<String> _cachedMetaTagNames = const {};
  Map<String, String> _cachedMetaTagCategories = const {};
  ...
  void initState() {
    super.initState();
    // Keep one merged listenable for the lifetime of the page. Recreating
    // Listenable.merge on every parent rebuild causes needless detach/attach
    // work in the tag manager, especially while the search field is active.
    _tagManagerListenable = Listenable.merge([
  ...
  Widget _buildSearchHeader() {
    ...
    final completions = _tagDraft.isEmpty
        ? <String>[]
        : store
              .allTags()
              .where((t) => t.toLowerCase().contains(_tagDraft.toLowerCase()))
              .take(8)
              .toList();
    return Padding(
              ...
              Expanded(
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    prefixIcon: Icon(
                      _globalMode ? Icons.search : Icons.filter_list,
                      size: 20,
                    ),
                ...
                child: Material(
                  ...
                  child: InkWell(
                    ...
                    onTap: () {
                      ...
                      _tags.clear();
                      _tagDraft = '';
                      setState(() => _globalMode = !_globalMode);
                    },
                    child: Padding(
  ...
  Future<void> _importLocalComics() async {
    ...
    final booksDir = Directory(
      '${supportDir.path}${Platform.pathSeparator}books',
    );
      ...
      try {
        ...
        if (await dest.exists()) {
          ...
          while (await dest.exists()) {
            dest = File(
              '${booksDir.path}${Platform.pathSeparator}$stem ($n)$ext',
            );
            n++;
          }
        }
    ...
    if (_tags.isNotEmpty) {
      list = list.where((r) {
        ...
        final assigned = <String>{
          ...?m?.tags,
          ...?m?.metaTags,
          ...store.tags.tagsForBook(key),
        };
        return _tags.every(assigned.contains);
      }).toList();
    }
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
  ...
  void _gotoTag(String tag) => setState(() {
    _section = 'tags';
    _detailTag = tag;
    _tagDetailFutureKey = null;
    _tagDetailFuture = null;
    _textSearch = '';
    _tags.clear();
    _tagDraft = '';
    _searchCtrl.clear();
  ...
  Widget _buildFilterBar(int count) => Container(
    ...
    child: Column(
      ...
      children: [
        Row(
          children: [
            const Icon(Icons.public, size: 16, color: Colors.amber),
              ...
              children: [
                if (_textSearch.isNotEmpty)
                  Chip(
                    ...
                    deleteIcon: const Icon(Icons.close, size: 14),
                    onDeleted: () {
                      _searchCtrl.text = _searchCtrl.text
                          .replaceFirst(_textSearch, '')
                          .trim();
                      _onSearch(_searchCtrl.text);
                    },
                    materialTapTargetSize: MaterialTapTargetSize.shrinkWrap,
  ...
  void _showEditSource(BookSource src) {
    final nameCtrl = TextEditingController(text: src.name),
        urlCtrl = TextEditingController(text: src.url ?? ''),
        ...
        pathCtrl = TextEditingController(text: src.path),
        noteCtrl = TextEditingController(text: src.note),
          ...
          child: Column(
            ...
            children: [
              ...
              if (src.isWebDav) ...[
                ...
                _fd('服务器地址', urlCtrl),
                _fd('端口(默认22)', portCtrl),
                _fd('用户名', userCtrl),
                _fdPw('密码', passCtrl),
                _fd('初始路径(默认/)', pathCtrl),
              ] else if (src.isBaidu) ...[
                _fd('根目录(默认/)', pathCtrl),
                _fdPw('refresh_token', tokenCtrl),
                _fd('AppKey(必填)', appKeyCtrl),
                _fdPw('SecretKey(必填)', secretCtrl),
              ] else if (src.is115) ...[
            ...
            onPressed: () async {
              ...
              await LibraryStore.instance.updateSource(
                ...
                refreshToken: tokenCtrl.text.trim().isEmpty
                    ? null
                    : tokenCtrl.text.trim(),
                clientId: appKeyCtrl.text.trim().isEmpty
                    ? null
                    : appKeyCtrl.text.trim(),
  ...
  Widget _fd(String label, TextEditingController c) => Padding(
  ...
  Widget _fdPw(String label, TextEditingController c) => Padding(
    padding: const EdgeInsets.only(bottom: 8),
    child: TextField(
      controller: c,
      obscureText: true,
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              _sourceTypeLabel(src),
              style: Theme.of(context).textTheme.bodySmall,
            ),
            Text(
              src.needsSession
                  ? (src.isBaidu || src.is115 || src.isQuark
                        ? src.path
                        : (src.url ?? ''))
                  : src.path,
              maxLines: 1,
  ...
  void _deleteSource(BookSource src) {
    showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text('将同时删除该书源下的所有阅读记录和元数据'),
        actions: [
          TextButton(
  ...
  // 标签管理
  // ============================================================
  Widget _buildTagManager() => ListenableBuilder(
    // Metadata/resource tags live in TagRepository, while BookMeta fields
    // live in LibraryStore. Listen to both so returning from a detail page
    // cannot leave the tag list showing a stale projection.
    ...
    builder: (c, _) {
      ...
      if (_tagManagerProjectionKey != projectionKey) {
        ...
        _cachedMetaTagNames = store.metaTagNames();
        _cachedMetaTagCategories = store.metaTagCategories();
      }
      final metaSet = _cachedMetaTagNames;
      // Filter before building the first frame so generated resource tags do
      // not flash while startup normalization is still in progress.
      final stats = _cachedTagStats
          .where(
            (s) => TagRepository.isVisibleInTagManager(
              s.$1,
              metadataNames: metaSet,
            ),
          )
      ...
      for (final t in metaTags) {
        metaGroups
            .putIfAbsent(_cachedMetaTagCategories[t.$1] ?? '类别', () => [])
            .add(t);
      }
          ...
          onSelected: (act) async {
            if (act == 'rename') {
              ...
              final ok = await showDialog<bool>(
                ...
                builder: (c) => AlertDialog(
                  title: Text('删除标签"${t.$1}"?'),
                  content: const Text('将从所有漫画中移除此标签'),
                  actions: [
                    TextButton(
              ...
              if (ok == true) LibraryStore.instance.deleteTag(t.$1);
            }
          },
        ),
      );
      ...
      return Row(
        children: [
          ...
          if (_detailTag != null) ...[
            const VerticalDivider(width: 1),
            Expanded(flex: 5, child: _buildTagDetail(_detailTag!)),
          ],
        ],
      );
    },
  ...
  Widget _buildTagDetail(String tag) {
    final cacheKey =
        '$tag:${LibraryStore.instance.revision}:${TagRepository.instance.revision}';
    if (_tagDetailFutureKey != cacheKey || _tagDetailFuture == null) {
      _tagDetailFutureKey = cacheKey;
      _tagDetailFuture = LibraryStore.instance.recordsByTag(tag);
    }
    final listF = _tagDetailFuture!;
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Padding(
  ...
  Widget _tabletLayout(AppSettings s) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      const Text(
        '平板布局',
      ...
      const SizedBox(height: 4),
      Text(
        '横屏建议"桌面风格"（与 PC 相同，侧栏导航）；竖屏建议"手机风格"（底部导航）。手机（宽度 <600dp）始终为手机风格。',
        style: Theme.of(context).textTheme.bodySmall,
      ),
  ...
  Widget _localComics(AppSettings s) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      const Text(
        '本地漫画',
      ...
      SwitchListTile(
        ...
        subtitle: const Text(
          '刷新本地 / SMB 书源时，后台将漫画文件夹 / zip 打包为 CBZ（需共享目录有写权限）；转换后与原内容视为同一本漫画（进度/标签保留）',
        ),
        dense: true,
  ...
  Widget _remoteSources(AppSettings s) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      const Text(
        '远程书源',
  ...
  Widget _readingDefaults(AppSettings s) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      const Text(
        '阅读默认',
      ...
      Wrap(
        ...
        children: [
          const Text('默认模式: '),
          DropdownButton<ReadMode>(
            value: s.readMode,
            items: ReadMode.values
      ...
      SingleChildScrollView(
        scrollDirection: Axis.horizontal,
        child: SegmentedButton<DualPageMode>(
          segments: DualPageMode.values
              .map((d) => ButtonSegment(value: d, label: Text(d.label)))
              .toList(),
          selected: {s.dualPageMode},
          onSelectionChanged: (vs) {
            s.dualPageMode = vs.first;
            LibraryStore.instance.updateSettings(s);
            setState(() {});
          },
        ),
      ...
      Wrap(
        ...
        children: [
          ...
          SizedBox(
            width: 120,
            child: Slider(
              value: s.dualPageGap.toDouble(),
              min: 0,
              max: 20,
              divisions: 20,
              label: '${s.dualPageGap}px',
              onChanged: (v) {
                s.dualPageGap = v.toInt();
                LibraryStore.instance.updateSettings(s);
                setState(() {});
              },
            ),
          ),
          Text('${s.dualPageGap}px'),
        ],
      ),
      const SizedBox(height: 8),
      Row(
  ...
  Widget _keybinds(AppSettings s) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      const Text(
        '自定义按键',
      ...
      const SizedBox(height: 4),
      Text('点击右侧按键标签后,按下新按键即可更改', style: Theme.of(context).textTheme.bodySmall),
      const SizedBox(height: 8),
      _keyRow('前进', s.keys.forwardKey, (k) {
        s.keys.forward = k.keyId;
        LibraryStore.instance.updateSettings(s);
        setState(() {});
      }),
      _keyRow('后退', s.keys.backKey, (k) {
        s.keys.back = k.keyId;
        LibraryStore.instance.updateSettings(s);
        setState(() {});
      }),
      _keyRow('放大', s.keys.zoomInKey, (k) {
        s.keys.zoomIn = k.keyId;
        LibraryStore.instance.updateSettings(s);
        setState(() {});
      }),
      _keyRow('缩小', s.keys.zoomOutKey, (k) {
        s.keys.zoomOut = k.keyId;
        LibraryStore.instance.updateSettings(s);
        setState(() {});
      }),
      _keyRow('缩放还原', s.keys.zoomResetKey, (k) {
        s.keys.zoomReset = k.keyId;
        LibraryStore.instance.updateSettings(s);
        setState(() {});
      }),
    ],
  ...
  Widget _keyRow(
    String label,
    LogicalKeyboardKey current,
    ValueChanged<LogicalKeyboardKey> fn,
  ) => Padding(
  ...
  Widget _coverQuality(AppSettings s) => Column(
    ...
    children: [
      ...
      const SizedBox(height: 4),
      Text('影响书架封面的扫描速度与清晰度', style: Theme.of(context).textTheme.bodySmall),
      const SizedBox(height: 10),
      SingleChildScrollView(
        scrollDirection: Axis.horizontal,
        child: SegmentedButton<CoverQuality>(
          segments: CoverQuality.values
...
class _KeyCaptureDialogState extends State<_KeyCaptureDialog> {
  ...
  Widget build(BuildContext c) => AlertDialog(
    title: const Text('按下新按键'),
    content: Focus(
      autofocus: true,
      onKeyEvent: (n, e) {
        if (e is KeyDownEvent) {
          setState(() => _k = e.logicalKey);
    ...
    actions: [
      ...
      FilledButton(
        onPressed: () => Navigator.of(c).pop(_k ?? widget.current),
        child: const Text('确认'),
      ),
    ],
  );
...
class AddSourceDialog extends StatefulWidget {
  ...
  @override
  State<AddSourceDialog> createState() => _AddDialogState();
}
...
class _AddDialogState extends State<AddSourceDialog> {
  ...
  Future<void> _submit() async {
    ...
    if (_type == 'webdav') {
      ...
      try {
        ...
        await LibraryStore.instance.addSource(
          BookSource(
            ...
            type: 'webdav',
            name: n.isEmpty ? _u.text.trim() : n,
            path: _b.text.trim().isEmpty ? s.root : _b.text.trim(),
            url: _u.text.trim(),
            username: _p.text.trim(),
            password: _s.text,
          ),
      ...
      if (path.isEmpty || !path.startsWith(r'\\')) {
        _setError('请填写 UNC 共享路径（以 \\ 开头，如 \\\\192.168.1.10\\comic）');
        return;
      }
      setState(() {
        _t = true;
      ...
      try {
        ...
        await LibraryStore.instance.addSource(
          BookSource(
            ...
            type: 'smb',
            name: n.isEmpty ? path : n,
            path: path,
          ),
        );
        if (mounted) Navigator.of(context).pop();
      } catch (e) {
        _setError('无法访问该共享目录:$e');
      } finally {
        if (mounted) setState(() => _t = false);
      }
    } else if (_type == 'baidu') {
  ...
  Future<void> _submitBaidu(String n) async {
    ...
    if (rt.isEmpty) {
      _setError('请先授权登录或粘贴 refresh_token');
      return;
    }
    setState(() {
      _t = true;
  ...
  Future<void> _submitQuark(String n) async {
    ...
    if (cookie.isEmpty) {
      _setError('请粘贴夸克网盘 Cookie（pan.quark.cn 登录后 F12 复制）');
      return;
    }
    setState(() {
      _t = true;
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (c) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            const Text(
              '浏览器已打开百度授权页，登录并同意后，把页面显示的授权码粘贴到这里。',
  ...
  Future<void> _cloud115Authorize() async {
    ...
    try {
      final qr = await cloud115QrStart(
        appId: _appId,
  ...
  Widget build(BuildContext c) => AlertDialog(
    ...
    content: ConstrainedBox(
      constraints: BoxConstraints(maxWidth: dialogMaxWidth(c)),
          ...
          children: [
            DropdownMenu<String>(
              ...
              dropdownMenuEntries: const [
                ...
                DropdownMenuEntry(
                  value: 'baidu',
                  label: '百度网盘',
                  leadingIcon: Icon(Icons.cloud_queue),
                ),
                DropdownMenuEntry(
                  value: '115',
                  label: '115 网盘',
                  leadingIcon: Icon(Icons.cloud_upload),
                ),
                DropdownMenuEntry(
                  value: 'quark',
                  label: '夸克网盘',
                  leadingIcon: Icon(Icons.cloud_done),
                ),
              ],
            ...
            if (_type == 'local')
              ...
              TextField(
                controller: _b,
                decoration: const InputDecoration(
                  labelText: '共享目录路径(UNC)',
                  hintText: r'\\192.168.1.10\comic',
                  border: OutlineInputBorder(),
                  isDense: true,
                ),
              )
              ...
              TextField(
                ...
                decoration: const InputDecoration(
                  labelText: '服务器地址',
                  hintText: '192.168.1.10 或 nas:2222',
                  border: OutlineInputBorder(),
                  isDense: true,
                ),
              ),
              ...
              const SizedBox(height: 10),
              OutlinedButton.icon(
                onPressed: _t ? null : _baiduAuthorize,
              ...
              TextField(
                ...
                obscureText: true,
                decoration: const InputDecoration(
                  labelText: 'refresh_token(授权后自动填入，也可直接粘贴)',
                  border: OutlineInputBorder(),
                  isDense: true,
                ),
              ),
              const SizedBox(height: 6),
              TextButton(
                onPressed: () => setState(() => _showAdv = !_showAdv),
                child: Text(_showAdv ? '收起高级选项' : '高级选项（必填 AppKey/SecretKey）'),
              ),
              if (_showAdv) ...[
                TextField(
                  controller: _appKey,
              ...
              const SizedBox(height: 10),
              OutlinedButton.icon(
                onPressed: _t ? null : _cloud115CookieAuthorize,
                icon: const Icon(Icons.qr_code_2),
                label: const Text('扫码获取 Cookie（无需 APP ID）'),
              ),
              const SizedBox(height: 6),
              TextField(
                controller: _cookie115,
                obscureText: true,
                decoration: const InputDecoration(
                  labelText: 'Cookie（扫码后自动填入，也可浏览器 F12 复制）',
                  hintText: 'UID=...; CID=...; SEID=...; KID=...',
                  border: OutlineInputBorder(),
                  isDense: true,
                ),
              ),
              ...
              TextButton(
                onPressed: () => setState(() => _showAdv = !_showAdv),
                child: Text(_showAdv ? '收起高级选项' : '高级选项（扫码设备 / 官方 APP ID 模式）'),
              ),
              if (_showAdv) ...[
                DropdownButton<String>(
                  ...
                  items: const [
                    DropdownMenuItem(
                      value: 'wechatmini',
                      child: Text('wechatmini（微信小程序，默认推荐）'),
                    ),
                ...
                OutlinedButton.icon(
                  onPressed: _t ? null : _cloud115Authorize,
                  icon: const Icon(Icons.qr_code),
                  label: const Text('官方模式：APP ID 扫码授权'),
                ),
                const SizedBox(height: 6),
                TextField(
                  controller: _token,
                  obscureText: true,
                  decoration: const InputDecoration(
                    labelText: 'refresh_token（官方模式，授权后自动填入）',
                    border: OutlineInputBorder(),
                    isDense: true,
                  ),
                ),
    ...
    actions: [
      ...
      FilledButton(
        onPressed: _t ? null : _submit,
        child: Text(_t ? '测试中…' : '添加'),
      ),
    ],
  );
}
...
class _QrScanDialog extends StatefulWidget {
  ...
  final void Function(int status, String? accessToken, String? refreshToken)
  onResult;
