...
- **离线智能刮削自动流程**：从本地目录快照生成 proposal，自动将无冲突的 Ready 结果写入元数据与标签，并接入既有同步队列；刮削过程不读取漫画内容，也不主动访问远程书源。
- **元数据详情展示**：漫画详情页显示刮削得到的作品名、作者、系列等信息，并保留可复制的原文件名。

### Fixed

- **标签投影与可读性**：作者、系列等元数据标签与资源标签在标签管理和漫画详情页保持一致，标签入口解析到真实资产后可正常阅读。
- **标签词汇收敛**：保留用户可理解的 `Chinese`、`无修正`、`高清` 等稳定标签，合并同义词并移除无意义的内部/投递标记。
- **书源刷新与删除清理**：新增/编辑书源无需重启即可刷新；115 根目录统一生效；远程对齐失败不会误删；确认删除后同步清理阅读记录、元数据、标签、AI 任务及缓存。
- **自动化运行稳定性**：修复重复刮削投影、资产身份碰撞和运行统计不一致问题，保留本地快照优先和零远程书源 I/O 边界。
