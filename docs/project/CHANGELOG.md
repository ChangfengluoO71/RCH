...
卷/话与 E 站元数据导入、条漫回跳修复（2026-09-23 并入本版发布）：

- **E 站自动刮削**（新）：「E 站自动刮削」设置项（按书源 + 文件夹批量识别，仅作品名唯一命中自动写）；
  纯函数匹配引擎（Dice + 阈值 0.5 + 同卷/并列保护 + 作品名锚点）；导入规划器（影子模式：命名空间前缀 +
  中文优先 + 只增不覆盖 + Ambiguous/Unmatched 零写入）；预览弹窗（命中依据如实带出）；
  标签中文翻译基线 1369 条（缺失按需更新）；manifest 语义层（`work_title`/`creators`/`source_series`/
  `censorship`/`tags_zh` 等）✓。识别率根因修复：候选源改实时搜索、匹配输入改用 M8 解析标题 ✓。
- **卷/话**：`book_metas` 物化 `volume`/`chapter`（解析值不再丢弃，空值不覆盖）✓；详情页信息区标题、
  刮削结果行、导入预览「匹配输入」统一显示「作品名 + 话号」，解析不到则不显示 ✓；
  两列随同步载荷与 `.rchpkg` 备份传播（`MetaSyncRow` 加 `serde(default)` 兼容旧包/旧节点）✓；
  修两条既有 merge 收敛缺陷（无 base 整条丢弃 metas 条目、合并结果未对齐落库状态致每轮重推）✓；
  **ADR-030 墓碑随行复活而失效**（导出读时过滤 + 应用时看时间 + 写活行即清墓碑）⇒ 整包恢复到全新库
  由 1055 行/111 章节回到 1154 行/卷 2/章节 177 ✓。
- **阅读器**：条漫快速下拉回跳修复（测高锚点补偿 `WebtoonAnchorKeeper` + `correctBy` 静默纠偏 +
  字节到达预告占位高度 + DEFUNCT 元素守卫）✓；分页改按视口推进（修末页配对越界永久转圈）✓；
  边界提示下沉 `_go()` 覆盖条漫与镜像方向 ✓；末页延迟 3 秒提示（再随机一本/退出详情页）✓。
- **其他**：修 codegen 导出私有类型（`frb(ignore)`）✓；分档阈值各 +200、页数默认 10、主页随机阅读入口 ✓。
...
- 修复文件与文件夹混合批量选择时目录项被展开为空、导致文件项也无法继续操作的问题。
- 文件夹批量标签写入离线索引时保留 `dir` 类型，避免目录被误记为文件。

### Improved

- 复用列表预检测、自动转换与批量标签展开之间的漫画文件夹检查结果，减少标签弹窗前的重复扫描等待。

## [0.5.4] — 2026-08-24

### Added

- **离线智能刮削**：从本地目录快照识别作品名、作者、系列、章节、合集和资源属性。
- **自动写入元数据与标签**：Ready 且无冲突的结果自动进入漫画元数据和标签。
- **详情页信息展示**：显示作品名、作者、系列、资源标签和可复制的原文件名。
- **可读资源标签**：提供 `Chinese`、`无修正`、`高清` 标签，并合并同义写法。
- **书源刷新与清理**：新增/编辑书源后自动刷新；确认删除时清理关联记录、元数据、标签和缓存。
- **重新刮削入口**：设置中提供统一的“重新刮削”操作。

### Fixed

- 修复元数据标签与详情页显示不一致，以及标签入口无法正常打开漫画的问题。
- 修复重复刮削产生重复标签/作品、覆盖用户手工元数据的问题。
- 修复扩展名不同的文件被错误合并、输入文件与 proposal 数量不一致的问题。
- 修复本地、夸克和 115 书源目录归一化导致的作品名覆盖和章节序列识别错误。
- 修复书源刷新失败时误清理本地数据的问题。
...
- **手机端阅读统计界面排版溢出**：维度切换（漫画/系列/标签/作者/类别）在窄屏改为标题独占一行 + 横向滑动，不再溢出；桌面/平板保持原有单行布局
...
- 标签详情书名显示离线索引真实文件名（id-path 书源不再显示 32hex 素材 id）

### Fixed

- **清理失效漫画数据无效（远程删除漫画后缓存与数据库残留）**：① 墓碑收集改用专用查询 `dbLoadLibraryIndexTombstones`（原接口 SQL `deleted=0` 过滤导致墓碑证据恒为空，前两版修复全部被该层拦截）；② 点击时先**在线核对远程书源**（枚举全树刷新离线索引，消失的漫画自动标记墓碑；核对失败的书源保守保留并提示）；③ 失效记录**联动删除元数据**（此前只删记录行，本地删文件/远程删漫画时标签、封面残留）与 page/raw/cover 缓存、AI 任务残留；④ 删除书源时逐本清缓存；⑤ 清理结果显示释放字节数与核对失败的源数
- **编辑元数据标签冲掉「已读」状态**：`updateMeta` 全量替换标签关联时保留「已读」（自动/手动独立维护的关联，不在手动标签集内）；手动取消过已读的书不会被自动恢复
