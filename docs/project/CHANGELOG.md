...
- **离线智能刮削自动流程**：从本地目录快照生成 proposal，使用文件名、父级/祖先目录和同目录序列推断作品、作者、章节与资源属性；无冲突的 Ready 结果自动写入元数据与标签，并接入既有同步队列。
- **元数据详情展示**：漫画详情页显示刮削得到的作品名、作者、系列、资源标签等信息，并保留可选择、复制的原文件名。

### Fixed

- **标签投影与可读性**：作者、系列等元数据标签与资源标签在标签管理和漫画详情页保持一致，标签入口通过真实资产解析后可正常阅读。
- **标签词汇收敛**：`中国翻訳`/`中文` 合并为 `Chinese`，`無修正` 保留为 `无修正`，`DL`/`DL版` 按项目语义归并为 `高清`；移除无意义的内部/投递标记，不再显示“数字版”。
- **书源刷新与删除清理**：新增/编辑书源无需重启即可刷新；115 根目录统一生效；远程对齐失败不会误删；确认删除后同步清理阅读记录、元数据、标签、AI 任务及缓存。
- **自动化运行稳定性**：修复重复刮削投影、资产身份碰撞和运行统计不一致问题；自动刮削只读取本地快照，保持零内容读取和零远程书源 I/O 边界。
