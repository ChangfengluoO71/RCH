# RCH 漫画阅读器 — 设计规范(SPEC) v2.1

> 本文档是 RCH 的**核心指导规范**。修订须经用户确认，详见第 14 节。

- 版本：v2.1（新增可选插件边界）
- 最近更新：2026-09-22
- 修订摘要：① v2.0 新增第 9 节"智能刮削与元数据架构（M8）"；M8-M1 catalog-only 离线提案层已完成 after8 版 347 条本地/夸克样本验证，保持提案模式，不自动写入 canonical metadata。② v2.1 经用户确认新增第 10 节"可选插件：外部订阅源"，明确插件与主程序的边界；原第 10~13 节顺延为第 11~14 节，第 13 节非目标中"不做在线漫画站爬虫/聚合"的适用范围同步收窄为**主程序**。
...
## 10. 可选插件：外部订阅源（v2.1 新增）

> 本节经用户确认新增（2026-09-22）。它不改变第 9 节的本地优先原则，只划定**插件**与主程序的边界。

### 10.1 定位

主程序**不做**在线漫画站的爬虫/聚合（见第 13 节）。但允许存在**默认关闭、用户显式启用**的可选插件，
用于从外部站点按规则筛选资源并**落盘到用户指定的文件夹**，例如「EH 订阅」：

```
外部站点（元数据 + 种子文件）
  -> 插件（可选、默认关闭、显式触发）
  -> 用户指定目录（.torrent / manifest.json）
  -> 用户自行决定后续（本地阅读 / 网盘离线下载 / 手动处理）
```

插件是**资源获取的旁路**，不是主程序的阅读链路。

### 10.2 强制边界（不可协商）

| # | 边界 | 说明 |
|---|---|---|
| B1 | **只读来源** | 插件只读外部站点的元数据接口与种子文件；**不得**读取图片、解析压缩包内容、或把图库当书源浏览 |
| B2 | **写出口唯一** | 插件只写**用户显式选择的目录**（种子文件 + 清单/manifest）；不得写入 canonical metadata、书架数据库、书源凭据或任何阅读数据 |
| B3 | **不进入主阅读链** | 插件产物若要被阅读，只能作为**普通本地/远程书源**被主程序发现，与插件本身解耦 |
| B4 | **默认关闭 + 显式触发** | 不随启动自动运行、不后台常驻轮询；必须由用户操作触发（手动点击 / 用户显式授权的计划任务） |
| B5 | **规则可编辑** | 筛选规则（站点查询语法、阈值、标签）以**用户可编辑的配置**为单一事实来源，不得硬编码为不可见常量 |
| B6 | **可解释** | 每轮运行必须给出分项统计（各条件拒绝数）与失败原因，禁止"静默筛空" |
| B7 | **限流优先** | 请求间隔可配且有下限；批量合并请求；失败快速失败，不做多端点×多页重试放大 |
| B8 | **凭据不入库不入包** | 若插件需要账号凭据，遵循既有凭据边界：本机保管、不进同步包（见第 13 节与 ADR 相关决策） |

### 10.3 与既有原则的关系

- 第 9.1 节「刮削不读远程书源」仍然成立：智能刮削与插件是两条独立通道，互不调用。
- 第一原则「阅读器永远只操作本地资源」仍然成立：插件不属于阅读器，其产物落盘后才可能成为阅读对象。
- 插件失败、被限流或站点改版，**不得影响**主程序的启动、阅读、同步与刮削。

### 10.4 验收要求

- 插件在**未启用**时对主程序零影响（无网络请求、无后台任务、无 UI 阻塞）。
- 插件运行期间主程序可正常阅读与浏览（网络与 IO 不得阻塞 UI 线程）。
- 插件写入范围可被静态核对：只出现用户所选目录下的文件。
- 规则变更不需要改代码即可生效。
...
- **主程序**不做在线漫画站爬虫/聚合（可选插件按第 10 节的强制边界另行约束，见第 10.1 节）
